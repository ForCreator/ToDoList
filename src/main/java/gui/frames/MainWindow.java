...
public class MainWindow extends JFrame{
	...
	private DataTask dataTask = new DataTask(HibernateInit.getSessionFactory());
	...
	public void init(){
		List<Task> list = dataTask.getAll();
		for (Task task : list) {
			...
			addTaskButton.addMouseListener(new MouseAdapter(){
				...
				public void mouseClicked(MouseEvent e) {
					if (getNewTaskField().getText().length() > 0) {
						String s = getNewTaskField().getText().trim();
						todoListModel.add(s);
						dataTask.save(s);
